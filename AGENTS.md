...
- `packages/eslint-plugin-format/` for the custom formatting plugin and rules.
- `packages/eslint-parser-plain/` for the parser package.
  Shared tooling lives in `scripts/` (e.g., type generation) and `tools/` (custom test helpers). TypeScript config variants are in `tsconfigs/`. Root-level configs like `eslint.config.ts`, `dprint.json`, and `cspell.config.yaml` define linting, formatting, and spelling rules.
